...
async function isEnrolled (req, res, next) {
    ...
    if (req.user.role !== 'student') {
        return res.status(403).json({ error: 'Only an enrolled student may perform this action'});
    }
    else {
        if (!req.resource) {
        ...
        if (!req.resource.courseId in req.user.coursesEnrolled) {
            return res.status(403).json({error: 'You do not have access to this resource'});
        }
        return next()
    }
...
module.exports = { isAdmin: isAdmin, isOwner: isOwner, isEnrolled };